...
class Beam(Truss):
    ...
    def __init__(self, position_list, coordinate_system_position, coordinate_system_orientation,thermal_load):
        ...
        if thermal_load:
            self.moment = ""#self.set_moment_of_x() - material_alpha * deltaT
        else:  
    ...
    def determine_correct_symbolic_line_load(self, line_load_string):
        ...
        l = self.length
        a = np.linspace(0, pi/2, 20) #erstellt zwischen 0 und pi/2 20 werte um die Kurve zu berechnen
        if line_load_string == "":
            return 0
